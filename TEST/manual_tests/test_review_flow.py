...
class ReviewFlowCases(LocalHttpTest):
    def test_B01_health_items_worker_routes(self):
        """B01: actual HTTP read routes expose grouped two-ROI sample and idle worker."""
    ...
    def test_B02_reject_only_selected_roi_and_save_feedback(self):
        """B02: selected ROI rejection queues that ROI; sibling's review stays pending."""
    ...
    def test_B03_approval_reaches_completed_after_mask_only(self):
        """B03: anomaly approval precedes mask approval; final workflow is completed."""
    ...
    def test_B04_mask_review_before_anomaly_approval_fails(self):
        """B04: mask approval before anomaly approval returns 400 and preserves state."""
        target = self.rows[0]["id"]
    ...
    def test_B05_bad_batch_prevalidation_has_no_partial_approval(self):
        """B05: batch containing an unknown ID does not approve its valid member."""
        target = self.rows[0]["id"]
        status, _ = self.request("POST", "/api/review-batch", {
    ...
    def test_B06_mask_save_binary_backup_and_size_validation(self):
        """B06: edited mask is binarized, backed up, reset pending; wrong dimensions are rejected."""
        target = self.rows[0]["id"]
    ...
    def test_B07_image_bytes_and_unknown_image(self):
        """B07: image endpoint returns actual source bytes; unknown item yields 404."""
        target = self.rows[0]["id"]
    ...
    def test_B08_csv_includes_independent_roi_outcomes(self):
        """B08: results download includes both ROI IDs and the saved rejection."""
        target = self.rows[0]["id"]
    ...
    def test_B09_bad_json_and_unknown_routes(self):
        """B09: malformed JSON returns 400; unknown route returns 404."""
    ...
    def test_B10_delete_requires_confirmation_and_is_recoverable(self):
        """B10: deletion requires exact ID and moves the whole synthetic part into trash."""
        target = self.rows[0]["id"]
    ...
    def test_B11_export_requires_all_roi_approvals(self):
        """B11: incomplete group is withheld; fully approved group exports one image and union mask."""
    ...
    def test_D03_anomaly_cannot_use_normal_stage_to_skip_mask_review(self):
        """D03: normal approval must be limited to normal samples."""
        target = self.rows[0]['id']
    ...
    def test_D03b_normal_cannot_use_anomaly_stage(self):
        """D03 reverse boundary: normal data should never enter the anomaly pipeline."""
        target = self.rows[0]['id']
